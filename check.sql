...
create or replace function 
	proj1_check_11(_type text, _name text, _res1 text, _res2 text, _query text) returns text
as $$
declare
	result1 text;
	result2 text;
begin
	result1 := proj1_check(_type, _name, _res1, _query);
	if result1 = _res1 || ' correct' then
		return result1;
	end if;
	
	result2 := proj1_check(_type, _name, _res2, _query);
	if result2 = _res2 || ' correct' then
		return result2;
	end if;
...
create or replace function 
	proj1_check_12(_type text, _name text, _res text, _query1 text, _query2 text) returns text
as $$
declare
	result1 text;
	result2 text;
begin
	result1 := proj1_check(_type, _name, _res, _query1);
	if result1 = _res || ' correct' then
		return result1;
	end if;
	
	result2 := proj1_check(_type, _name, _res, _query2);
	if result2 = _res || ' correct' then
		return result2;
	end if;
...
as $chk$
select proj1_check_11('view','q11','q11_expected', 'q11_old_expected',
									 $$select * from q11$$)
$chk$ language sql;
...
as $chk$
select proj1_check_12('function','q12','q12a_expected',
									 $$select q12(54233, 11)$$, $$select q12(54233, 3)$$)
$chk$ language sql;

create or replace function check_q12b() returns text
as $chk$
select proj1_check_12('function','q12','q12b_expected',
									 $$select q12(63045, 5)$$, $$select q12(63045, 3)$$)
$chk$ language sql;

create or replace function check_q12c() returns text
as $chk$
select proj1_check_12('function','q12','q12c_expected',
									 $$select q12(49477, 20)$$, $$select q12(49477, 17)$$)
$chk$ language sql;

create or replace function check_q12d() returns text
as $chk$
select proj1_check_12('function','q12','q12d_expected',
									 $$select q12(48819, 442)$$, $$select q12(48819, 380)$$)
$chk$ language sql;

create or replace function check_q12e() returns text
as $chk$
select proj1_check_12('function','q12','q12e_expected',
									 $$select q12(61087, 71)$$, $$select q12(61087, 57)$$)
$chk$ language sql;
...
drop table if exists q11_old_expected;
create table q11_old_expected (
	unsw_id integer
);
...
COPY q11_old_expected (unsw_id) FROM stdin;
3232152
3144015
3278476
3219452
